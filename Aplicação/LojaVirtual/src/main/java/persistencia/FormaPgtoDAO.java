...
public class FormaPgtoDAO implements Serializable{
	...
	public static FormaPagto pesqId(int valor) {
		Session sessao = HibernateUtil.getSessionFactory().openSession();
		Query consulta = sessao.createQuery("from FormaPagto where fpg_id = :parametro");
		consulta.setInteger("parametro", valor);
		sessao.close();
		return (FormaPagto) consulta.uniqueResult();
	}
